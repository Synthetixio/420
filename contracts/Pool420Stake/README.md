...
forge create \
  ...
  --verify \
  src/Pool420Stake.sol:Pool420Stake \
  --constructor-args \
      $_CoreProxy \
      $_AccountProxy \
...
# Get the readable abi and update importPool420Stake.ts
node ../../readableAbi.js "$(cat ./out/Pool420Stake.sol/Pool420Stake.json | jq -c '.metadata.output.abi')"
```
...
export _Pool420Stake="0x6C3F7ed79b9D75486D0250946f7a20BDA74844Ba"

cast selectors $(cast code $_Pool420Stake)

cast call $_Pool420Stake 'function AccountProxy() view returns (address)'
cast call $_Pool420Stake 'function CoreProxy() view returns (address)'
cast call $_Pool420Stake 'function LegacyMarketProxy() view returns (address)'
cast call $_Pool420Stake 'function TreasuryMarketProxy() view returns (address)'
cast call $_Pool420Stake 'function V2xResolver() view returns (address)'

cast call $_Pool420Stake 'function get$SNX() view returns (address $SNX)'
cast call $_Pool420Stake 'function get$sUSD() view returns (address $sUSD)'
cast call $_Pool420Stake 'function get$snxUSD() view returns (address $snxUSD)'
cast call $_Pool420Stake 'function getV2x() view returns (address v2x)'
cast call $_Pool420Stake 'function getV2xUsd() view returns (address v2xUsd)'
...
cast send --unlocked --from $_owner $_CoreProxy "function setConfig(bytes32 k, bytes32 v)" \
  ...
  "$(cast to-uint256 1)"
cast send --unlocked --from $_owner $_CoreProxy "function setConfig(bytes32 k, bytes32 v)" \
  "$(cast keccak "$(cast abi-encode "f(bytes32,address,uint128)" $_senderOverrideMinDelegateTime $_Pool420Stake "$(cast to-uint256 1)")")" \
  "$(cast to-uint256 1)"
cast send --unlocked --from $_owner $_CoreProxy "function setConfig(bytes32 k, bytes32 v)" \
  ...
  "$(cast to-uint256 1)"
cast send --unlocked --from $_owner $_CoreProxy "function setConfig(bytes32 k, bytes32 v)" \
  "$(cast keccak "$(cast abi-encode "f(bytes32,address,uint128)" $_senderOverrideWithdrawTimeout $_Pool420Stake "$(cast to-uint256 1)")")" \
  "$(cast to-uint256 1)"
...
export _SNX=$(cast call $_Pool420Stake 'function get$SNX() view returns (address $SNX)')
export _sUSD=$(cast call $_Pool420Stake 'function get$sUSD() view returns (address $sUSD)')
export _snxUSD=$(cast call $_Pool420Stake 'function get$snxUSD() view returns (address $snxUSD)')
...
cast send --unlocked --from 0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266 $_SNX "function approve(address spender, uint256 amount) returns (bool)" $_Pool420Stake $(cast to-wei 10000)
cast send --unlocked --from 0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266 $_Pool420Stake "function setupPosition(uint256 snxAmount)" $(cast to-wei 10000)
...
cast call --from 0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266 $_Pool420Stake 'function getTotalDeposit() view returns (uint256 totalDeposit)'
cast call --from 0xf39fd6e51aad88f6f4ce6ab8827279cfffb92266 $_Pool420Stake 'function getTotalLoan() view returns (uint256 totalLoan)'
cast call --from 0xc3Cf311e04c1f8C74eCF6a795Ae760dc6312F345 $_Pool420Stake 'function getTotalDeposit() view returns (uint256 totalDeposit)'
```
