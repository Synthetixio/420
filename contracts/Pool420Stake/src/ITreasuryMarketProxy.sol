...
interface ITreasuryMarketProxy {
    ...
    event DepositRewardIssued(
        uint128 indexed accountId, address indexed rewardToken, ITreasuryMarket.LoanInfo depositedRewardData
    );
    event DepositRewardRedeemed(
        uint128 indexed accountId, address indexed rewardToken, uint256 rewardRedeemed, uint256 penaltyPaid
    );
    ...
    function depositRewardConfigurations(uint256)
        external
        view
        returns (
            address token,
